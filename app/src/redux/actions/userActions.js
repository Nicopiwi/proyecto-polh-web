      ...
      dispatch({
        ...
        payload: {
          ...
          userNombreEstablecimiento:
            type === "farmacia" ? res.user.nombreEstablecimiento : null,
...
export const modifyUserName = (userName, userSurname, userType) => (
  dispatch
) => {
  const uri =
    userType === "medico" ? APIs.rest.updateMedico : APIs.rest.updateFarmacia;
  let headers = new Headers();
  headers.append("Content-Type", "application/json");
  headers.append("Accept", "application/json");
  headers.append("Access-Control-Allow-Origin", "*");
...
export const modifyMatricula = (userMatricula) => (dispatch) => {
  ...
  return fetchTimeout(
    uri,
    {
      method: "PUT",
      headers,
      body: JSON.stringify({ matricula: userMatricula }),
    },
    30000
  )
    .then((res) => res.json())
...
export const modifyEmail = (userEmail, userType) => (dispatch) => {
  ...
  return fetchTimeout(
    uri,
    { method: "PUT", headers, body: JSON.stringify({ email: userEmail }) },
    30000
  )
    .then((res) => res.json())
    .then((res) => {
...
export const modifyDireccion = (userDireccion) => (dispatch) => {
  ...
  return fetchTimeout(
    uri,
    {
      method: "PUT",
      headers,
      body: JSON.stringify({ direccion: userDireccion }),
    },
    30000
  )
    .then((res) => res.json())
...
export const modifyNombreEstablecimiento = (userNombreEstablecimiento) => (
  dispatch
) => {
  const uri = APIs.rest.updateFarmacia;
  let headers = new Headers();
  ...
  return fetchTimeout(
    uri,
    {
      ...
      headers,
      body: JSON.stringify({
        nombreEstablecimiento: userNombreEstablecimiento,
      }),
    },
    7000
  )
